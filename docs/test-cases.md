...
| TC-WS-010 | Manual dark toggle | Theme button is clicked | `toggleWorkspaceTheme()` called | `data-ws-theme` attribute is set on `<html>`; preference stored in `localStorage.setItem("workspace-theme", …)`; button label reflects mode. | 상태전이 | Browser (Cycle 32) |
| TC-WS-011 | Card selected state | User checks a card checkbox | `_wsOnCardToggle` fires | `ws-card-selected` class added to card item; class removed on uncheck. | 상태전이 | Browser (Cycle 32) |
| TC-WS-012 | Card hover state | User hovers card | Mouse enter | `--ws-accent-tint` background applied via `.ws-card-item:hover` rule. Hover render not asserted in headless — CSS rule verified by source inspection. ⚠ | 기능 테스트 | Source inspection; hover event browser (Cycle 32) |
| TC-WS-013 | Keyboard focus on interactive controls | Keyboard tab | `:focus-visible` | Tab navigation from a known starting point reaches checkbox, target fields, and generate button; checkbox label, disclosure `aria-controls`, and theme toggle text verified. Custom outline verified by source inspection. | 기능 테스트 | Browser (Cycle 32): Tab nav + ARIA |
...
| TC-WS-015 | Fit signal — coverage percentage | Target text and selected card share keywords | Computed intersection | `ws-coverage-value` shows `pct%` where `pct = |T∩C|/|T|×100`. | 기능 테스트 | Browser (Cycle 32) |
| TC-WS-016 | Fit signal — target input listeners | User types in target field | `input` event | `_wsWireTargetListeners` wires `input` listener to all 5 target fields; `_wsUpdateCoverage` fires on each change. | 기능 테스트 | Browser (Cycle 32) |
| TC-WS-017 | Card disclosure — line-clamp | Summary is long | Rendered card | `ws-card-more` button shown when `scrollHeight > clientHeight`; deterministic at 320 px viewport with long-summary fixture — no skips, no DOM mutation. | 기능 테스트 | Browser (Cycle 32) |
| TC-WS-018 | Card disclosure — expand/collapse | User clicks "Show full summary" | Button click | `ws-card-context-expanded` class applied; button text switches to "Collapse summary"; expanded height > collapsed height. | 상태전이 | Browser (Cycle 32) |
| TC-WS-019 | Disclosure stops propagation | User clicks `ws-card-more` | `event.stopPropagation()` called | Card-selection checkbox is not toggled by disclosure click. | 기능 테스트 | Browser (Cycle 32) |
| TC-WS-020 | Preview payload unchanged | Workspace generates preview | `generateWorkspacePreview()` called | `POST /api/studio/application-preview` with `output_type`, `card_ids`, `target_context` captured and asserted; card file count unchanged. | 회귀/기능 테스트 | Browser (Cycle 32) |
