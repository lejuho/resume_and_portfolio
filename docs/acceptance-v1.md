...
| 5 | `--jd <jd> --tone formal` → LLM rewrite PDF 반영 | PDF 내 요약문이 JD 기반으로 변경됨 확인 | `uv run pcli build resume --jd tests/fixtures/fake-jd.txt --tone formal` (requires `ANTHROPIC_API_KEY`; dry-run 확인은 `--dry-run` 추가) | Yes | |
