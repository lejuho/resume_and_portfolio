...
| 5 | `--jd <jd> --tone formal` → LLM rewrite PDF 반영 | PDF 내 요약문이 JD 기반으로 변경됨 확인 | `uv run pcli build resume --jd tests/fixtures/fake-jd.txt --tone formal` (requires a configured supported AI provider key; dry-run 확인은 `--dry-run` 추가) | Yes | |
...
AI provider configured: mock / anthropic / google
Provider key set: Yes / No  (never record the key value)
...
- JD file used: `tests/fixtures/fake-jd.txt` (dry-run 확인용) or custom JD
- Tone: formal
