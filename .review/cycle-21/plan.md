...
## Escalation Amendment v3: Unified Blind-Hiring Boundary

Status: Approved after `review-v6.md` identified repeated field-by-field blind-hiring leaks.

This amendment supersedes any narrower blind-hiring implementation guidance in v1/v2 where
the rules conflict. It does not add a new product feature: it defines one safe data boundary
required to complete the existing blind-hiring contract.

### Problem

Previous fixes screen individual known fields, but raw selected-card data and raw provider
advisory values can still reach downstream preview or provider surfaces through newly found
fields such as canonical card identifiers. Patching each surfaced field separately does not
satisfy the contract that excluded personal-background content is absent from transmitted and
rendered outputs.

### Policy Decisions

- Protected content means identity/background information derived from selected career cards
  or generated by the provider.
- User-supplied `target_context` is application context, not applicant-fact leakage. It may
  remain visible in `target_context_used` and may be sent as target context to the provider.
- User-supplied context must not be converted into an asserted applicant personal-background
  fact in `personal_facts` or the verified `answer_draft`.
- Under `blind_hiring=true`, protected card/provider content must not enter provider fact
  payloads, cache fact identity, or any rendered preview field.
- If sanitization leaves at least one usable card fact, preview generation continues with a
  visible warning. If no usable card fact remains, return HTTP `422` and do not call a
  provider.

### Required Design

- Build one server-owned `safe_projection` before either mock or LLM processing.
  It contains sanitized ledger facts, safe selected-card display data, opaque references,
  warning codes, and an internal-only provenance mapping.
- Once `safe_projection` is built, neither mock nor LLM flow may read raw selected cards again
  while composing the preview or verified draft.
- Screen all card-derived transmitted or rendered material when blind hiring is enabled,
  including title, summary, metrics, evidence, canonical card ID, and rationale source text.
- In blind mode, provider fact payloads and returned preview provenance must use opaque
  references such as `F1` and `C1`; raw canonical card IDs must remain server-internal only.
  Non-blind response behavior may retain its existing canonical-ID compatibility.
- Provider advisory is untrusted input. Sanitize it into `safe_advisory` before preview
  assembly; preview construction must never read raw advisory visible strings directly.
  Sanitization covers `question_intent`, `competency_target`, `missing_info[].message`,
  `ai_guidance[]`, and future provider-derived visible string fields.
- Mock and LLM paths must assemble `fact_ledger`, `selected_cards`, `personal_facts`, and
  `answer_draft` from the same `safe_projection`. Provider output remains advisory-only and
  must not supply copyable draft prose.

### Interface Decisions

- In blind mode, `fact_ledger` and `selected_cards` expose safe opaque card provenance rather
  than canonical card IDs; UI continues to display safe `display_title` and
  `selection_reason`.
- If all selected-card facts are removed by blind-hiring sanitization, the endpoint returns
  HTTP `422` with a safe message equivalent to "selected evidence cannot safely support a
  blind-hiring preview."
- Existing warning codes remain valid; projection redaction and advisory redaction must be
  distinguishable in `missing_info`.

### Required Tests

- Identity-bearing canonical card ID is absent from the captured provider fact payload and the
  serialized blind preview; opaque references and safe display labels remain present.
- Identity-bearing title, summary, metric, and evidence content is absent from both mock and
  LLM paths.
- Identity-bearing provider strings are absent across every advisory-visible field after
  sanitization.
- Identity-bearing user-supplied target context remains visible only as permitted target
  context and is not emitted as an applicant fact or personal-background claim in the verified
  draft.
- Whole-payload assertions verify that protected card/provider markers are absent from the
  captured provider fact projection and returned preview JSON in adversarial blind-hiring
  cases.
- Sanitization with partial safe evidence succeeds with warnings; sanitization that removes
  all usable evidence returns HTTP `422` and makes no provider call.
- Existing provenance, cache isolation, fallback visibility, no-persistence, provider
  advisory separation, and copy-action regression tests remain green.

### Executor Handoff

- Preserve the ISSUE-8/ISSUE-9 fixes already appended to `review-v6.md`; extend them into the
  unified projection boundary rather than reverting them.
- Do not modify the Codex-authored body or appended `RESOLVED` section of `review-v6.md`.
- Implement only this amended boundary scope, run the plan's full automatic checks, and
  record the next step-advisor artifact under `advisor-feedback/`.
- Leave final cycle verdict and any `ready_to_merge` status change to the next independent
  Cycle Reviewer, which must write `review-v7.md`.

---

## Escalation Amendment v4: Studio Live-Card Selector Contract Fix

Status: Approved after `review-v10.md` identified a merge-blocking UI integration regression.

This amendment does not reopen Amendment v3 or introduce new product scope. ISSUE-12 is
limited to the Studio Application Writing selector consuming the existing `/api/cards`
response contract incorrectly.

### Decision

- Do not split Cycle 21 and do not rewrite the full plan.
- Preserve the existing `GET /api/cards` JSON array response contract.
- Fix only ISSUE-12 before final merge review.

### Required Change

- Update `scripts/static/studio.js` so `loadAppCards()` consumes the existing `/api/cards`
  array response.
- Preserve the current `status === "live"` filtering behavior.
- Preserve the existing empty-state and request-error UI behavior.

### Required Test And Verification

- Add automated regression coverage proving that when `/api/cards` returns a live card, the
  Application Writing selector contract can display that card rather than rendering the
  false empty state.
- Run the full required automatic check set after implementation.

### Executor Handoff

- Implement only ISSUE-12 from `review-v10.md`.
- Do not modify the Codex-authored body of `review-v10.md`; append exactly one `RESOLVED`
  section after implementation.
- Record the next step-advisor artifact under `advisor-feedback/`.
- Leave final verdict and any `ready_to_merge` status change to the next independent Cycle
  Reviewer, which must write `review-v11.md`.
